...
import { loadConfig, mergeConfigs } from '../../config/index.js';
...
export const scanCommand = new Command('scan')
    ...
    const finalEnabledIds = enabledDetectorIds.filter(id => !(config.detectors.disabled?.includes(id)));
    // Register enabled detectors
    for (const id of finalEnabledIds) {
        const detector = allDetectors[id];
        if (detector) {
