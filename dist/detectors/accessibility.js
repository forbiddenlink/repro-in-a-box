...
export class AccessibilityDetector extends BaseDetector {
    id = 'accessibility';
    name = 'Accessibility';
    description = 'Detects WCAG 2.1 Level A & AA accessibility violations using axe-core';
    category = IssueCategory.ACCESSIBILITY;
    async attach(_page, _config) {
        ...
        try {
            ...
            for (const violation of violations) {
                ...
                for (const node of violation.nodes) {
                    this.addIssue(this.createIssue(`a11y-${violation.id}`, `${violation.help}: ${node.failureSummary || violation.description}`, severity, page.url(), {
