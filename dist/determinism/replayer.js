 ...
 */
export async function replayFromHar(options) {
    const errors = [];
    ...
    try {
        ...
        const registry = options.scanner.registry;
 ...
 */
export async function validateReproducibility(options) {
    ...
    const originalScan = JSON.parse(await fs.readFile(originalScanPath, 'utf-8'));
    // Find HAR file
 ...
 */
function calculateReproducibilityScore(original, replays) {
