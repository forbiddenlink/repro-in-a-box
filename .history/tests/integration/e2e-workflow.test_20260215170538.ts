...
describe('E2E Workflow', () => {
  ...
  it('should complete full scan workflow', async () => {
    ...
    await registry.loadBuiltInDetectors();

    const scanner = new Scanner(registry);
    
    const results = await scanner.scan(TEST_SITES.simple, {
      screenshots: false,
      crawler: {
        maxDepth: 1,
        maxPages: 1,
  ...
  it('should create bundle with HAR and validate', async () => {
    ...
    const validation = await validateReproducibility(harPath, {
    ...
    expect(validation.runs).toHaveLength(2);
    expect(validation.runs[0].issues).toBeDefined();
  ...
  it('should detect issues consistently', async () => {
    ...
    // Run scan twice
    const results1 = await scanner.scan(TEST_SITES.simple, {
      screenshots: false,
      crawler: {
        maxDepth: 1,
        maxPages: 1,
    ...
    const results2 = await scanner.scan(TEST_SITES.simple, {
      screenshots: false,
      crawler: {
        maxDepth: 1,
        maxPages: 1,
